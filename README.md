...
- **配置修改智能感知（Dirty State Tracking）**：只有当输入或选项发生实际修改时，「保存并应用」按钮才会高亮激活；未修改时自动置灰禁用；
- **一键清理配置与绿色卸载**：支持在设置中心一键清除全部注册表与开机自启项，并支持 `--clean` / `--uninstall` 命令行参数；
- **键盘回车快捷保存**：在设置中心按 `Enter` 键即可直接保存并应用配置，按 `Esc` 键快速关闭；
- **用户友好提示**：内置全控件 Tooltips 气泡帮助说明，支持提示音开关、开机自启、边缘吸附与窗口置顶开关。
...
└── tests/
    ...
    └── test_statemachine.cpp      # 15 项核心状态机、时钟锚定、配置、脏状态对比与边框线宽单元测试套件
```
